...
from rmsd_analysis import calculate_rmsd
...
class TwodRMSD:
    ...
    def calculate_2d_rmsd(self, trial):
        ...
        ref_gr_u = mda.Universe(self.ref_gr)
        ref_alt_u = mda.Universe(self.ref_alt)

        rmsd_gr = calculate_rmsd(universe, ref=ref_gr_u, align_range='backbone', analysis_range=self.input_dict['rmsd_range'])
        rmsd_alt = calculate_rmsd(universe, ref=ref_alt_u, align_range='backbone', analysis_range=self.input_dict['rmsd_range'])

        rmsd_2d_data = np.array([rmsd_gr[self.input_dict['rmsd_range']], rmsd_alt[self.input_dict['rmsd_range']]]).T
        return rmsd_2d_data

    def fit_and_filter_data(self, rmsd_2d_data, threshold=3):
    ...
    def plot_filtering_data(self, rmsd_2d_data):
        ...
        plot_path = f"{self.input_dict['output_path']}/plots/{self.input_dict['jobname']}_{self.input_dict['max_seq']}_{self.input_dict['extra_seq']}_2d_rmsd_fit.png"
    ...
    def plot_and_save_2d_data(self):
        ...
        title = f"{self.input_dict['jobname']} {self.input_dict['max_seq']} {self.input_dict['extra_seq']} Score: {self.filtering_dict['ratio']:.2f}"
        ...
        plot_path = f"{self.input_dict['output_path']}/plots/{self.input_dict['jobname']}_{self.input_dict['max_seq']}_{self.input_dict['extra_seq']}_2d_rmsd_clustered.png"
        plt.savefig(plot_path, dpi=300)
        plt.close()
        ...
        for i in unique_labels:
            records.append({
                ...
                'rmsd_range': self.input_dict['rmsd_range'],
        ...
        csv_path = f"{self.input_dict['output_path']}/datasets/{self.input_dict['jobname']}_clustering_analysis.csv"
        df.to_csv(csv_path, index=False)

    def get_2d_rmsd(self, rmsd_mode_df_path):
        ...
        for trial in unique_trials:
            ...
            if self.ref_gr is None:
                self.ref_gr = trial_subset['pdb_filename'].iloc[0]
            if self.ref_alt is None:
                self.ref_alt = trial_subset['pdb_filename'].iloc[-1]
